...
Cait can't have just one chocolate a day. So our next task would be to create a function `dispenseChocolates()` that dispenses chocolate at her will. It takes `chocolates`, `number` as an argument and _returns an array of chocolates_ from the bottom.
...
In case of count value more than number of chocolates we have to return 'Insufficient chocolates in the dispensers' 

### Progression 5: LED my chocos!

Cait is finding it difficult to keep track of her chocolates. You'll need to create an LED that shows her how many chocolates of each color are remaining. Create a function `noOfChocolates()`, that takes candies as argument and  that _returns an array of number of chocolates_ in the order `[green, silver, blue, crimson, purple, red, pink]`.
