...
function App() {
  ...
  function reducer(posts, action) {
    switch(action.type) {
      ...
      case ACTION_TYPE.TOGGLE:
        ...
        return posts.map(post => post.id === action.id ? { ...post, toggle: !post.toggle } : post);
      default:
        return posts;
    }
  }
  ...
  function handleSubmit(e) {
    ...
    dispatch({ type: ACTION_TYPE.ADD_POST, payload: newPost(name) });
    setName(" ");
  }
  ...
  return(
    <div>
...

      {
        ...
        posts.map(post => <NewPost key={post.id} post={post} dispatch={dispatch} />)
      }
